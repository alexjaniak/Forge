...
def cmd_kill(args):
    if args.all == bool(args.id):
        print("Error: pass either an agent id or --all", file=sys.stderr)
        sys.exit(1)

    if args.all:
        killed = kill_managed_runs()
        if not killed:
            print("No running managed agents found")
            return
        for run in killed:
            print(f"Killed {run['agent_id']} (pid {run['pid']}) with {run['signal']}")
        return

    killed = kill_managed_runs(args.id)
    if not killed:
        print(f"No running managed agent found for '{args.id}'")
        return
    for run in killed:
        print(f"Killed {run['agent_id']} (pid {run['pid']}) with {run['signal']}")
...
def main():
    ...
    p_kill = sub.add_parser("kill", help="Kill one or all managed runs")
    p_kill.add_argument("id", nargs="?", help="Managed agent id to kill")
    p_kill.add_argument("--all", action="store_true", help="Kill all managed runs")
