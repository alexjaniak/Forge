...
| **IssuesPanel** | `issues-panel.tsx` | Polls `/api/issues` every 5s. Shows GitHub issues with label badges (color-coded by status/role/type). Filterable by status and role labels. Audio alert for new `role:admin` issues. |
