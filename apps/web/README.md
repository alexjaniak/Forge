...
| **AgentPanel** | `agent-panel.tsx` | Lists agents with status badges (NEW, ACTIVE, MODIFIED, DELETED), role badges, interval/countdown info, and branch context. Supports add, delete, force-run, apply, clear, and reset actions. Auto-refreshes every 5s and respects the dashboard refresh trigger. |
| **LogsPanel** | `logs-panel.tsx` | Streams logs via SSE (`/api/logs/stream`) with polling fallback (5s). The dashboard refresh control triggers an immediate refresh even when Logs is not the active tab. Tabs for each agent plus an "All" view. Parses buffered `=== RUN <timestamp> duration=<N>s exit=<code> ===` entries plus `=== SKIP <timestamp> reason=<...> ===` markers. Max 200 blocks displayed. |
